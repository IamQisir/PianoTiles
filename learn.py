...
# Return an object containing information about the current display settings
info = pygame.display.Info()
width = info.current_w
height = info.current_h

# Set the display mode based on the current display settings
if width >= height:
    win = pygame.display.set_mode(SCREEN, pygame.NOFRAME)
else:
    # Set the display mode to fullscreen and scaled mode if the height is greater than the width
    # "|" is a bitwise OR operation, combining multiple flags into one
...
running = True
while running:
    # pos is the position of the mouse cursor when the user clicks the mouse button
    pos = None
    ...
    # fill the screen with the background image to refresh the display
    win.blit(bg_img, (0, 0))
    square_group.update()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE or event.key == pygame.K_q:
                running = False
		# the pygame.MOUSEBUTTONDOWN event is triggered when the user clicks the mouse button
        # and the event.pos attribute contains the position of the mouse cursor at the time of the click
    ...
    if home_page:
        ...
        if pos and start_rect.collidepoint(pos):
            ...
            # before the game starts, create a tile object at a random x position
    ...
    if game_page:
        ...
        if time_counter.count <= 0:
            # update every single tile in the tile group seperately
            for tile in tile_group:
                tile.update(speed)

                if pos:
                    # check if the mouse cursor is within the tile's rectangle
            ...
            if len(tile_group) > 0:
                ...
                if t.rect.top + speed >= 0:
                    x = random.randint(0, 3)
                    # ensure the no spacing between tiles
